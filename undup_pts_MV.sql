	...
	from 
		...
		AND pt_max_enc_date_loc.location_id = '8250AA0C-BCF3-42CB-9338-ED25F1049571'
		GROUP BY person.person_id
		HAVING COUNT (patient_encounter.billable_timestamp) >= 2) as ascvd_pts_MV
		
		
	
	
	SELECT * INTO #dm_pts_MV
	FROM
		(SELECT DISTINCT person.person_id
		FROM person
		INNER JOIN patient_encounter ON patient_encounter.person_id = person.person_id
		...
		AND pt_max_enc_date_loc.location_id = '8250AA0C-BCF3-42CB-9338-ED25F1049571'
		GROUP BY person.person_id
		HAVING COUNT (patient_encounter.billable_timestamp) >= 2) as dm_pts_MV
		
		
		
		SELECT * 
		FROM #htn_pts_MV
		FULL OUTER JOIN #ascvd_pts_MV on #ascvd_pts_MV.person_id = #htn_pts_MV.person_id 
		FULL OUTER JOIN #dm_pts_MV on #dm_pts_MV.person_id = #htn_pts_MV.person_id
		WHERE #htn_pts_MV.person_id is NULL OR (#ascvd_pts_MV.person_id is NULL AND #dm_pts_MV.person_id is  NULL)
